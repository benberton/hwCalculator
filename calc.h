...
private:
    ...
    */
    ...
    /*
    pre: invoked by CheckTokens()
    post: returns true if char is an operator
    */
    bool IsOperator(char);

    /*
    pre: invoked by CheckTokens()
    post: returns true if char is an uppercase char
    */
    bool IsChar(char);

    /*
    pre: invoked by CheckTokens()
    post: returns true if char is a digit
    */
    bool IsDigit(char);

    /*
    pre:  invoked from constructor
    post: 26 position valueTbl is dynamically allocated and filled with zeroes
      ...
      2. inFix is a copy of argv[1] except:
         ...
         valueIdx is appropriately incremented.
             Example: ((322+12)*12) becomes ((A+B)*C)
              ...
              subtracted from the operands, we get the index
              ...
              in the next available position in valueTbl:
                  [322 12 12 334 0 ... 0]
              The upper alphabetic character, corresponding to the position of
    ...
    */
    ...
    /*
     pre:  Null terminated array exist
     post: Returns size of array, less the null terminator
    */
    int GetLength(char* array);

    /*
     pre:  Array exist with a start point and end point
     post: Number held from start to end point added to infix
    */
    void AddToValueTbl(int start,int end);
   ...
   */
    ...
    /*
    pre: a desired operation, a first number, and a second number exist
    post: returns the first number and the second number with the desired operation
    */
    int GetTotal(char operation, int num1, int num2);
